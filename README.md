...
- **增强思维导图JSON解析**:
  ...
  - **动态层级结构转换**: 自动将所有非标准字段转换为子节点,不限定于固定字段名
  - 支持任意字段名(如`欲望`、`阻碍`、`背景`、`冲突`、`高潮`等),完全由AI返回内容决定
  - 丰富思维导图的层级结构,每个事件自动展开为多层级的详细视图
  - 添加详细的调试日志,便于诊断解析失败的原因
  - 修复"AI返回数据但显示为空"的问题

- **新增预览确认功能** ⭐ 重要更新:
  - **真正的流式API**: 使用Gemini的`generateContentStream`，像打字一样逐字显示
  - **立即弹出预览**: 点击重绘后立即显示预览窗口，无需等待生成完成
  - **实时流式响应**: AI生成过程中实时显示JSON响应，逐字符更新
  - **可视化进度**: 窗口顶部显示任务进度条和状态提示
  - **全屏预览**: 预览Modal扩大到98%视口，提供更大的查看空间
  - **原始响应显示**: 新增"原始响应"标签页，实时显示AI返回的JSON文本
  - **一键复制**: 可以直接复制原始JSON响应，方便调试和分析
  - **三标签页切换**: 原始响应、原内容、新内容三个标签页，方便对比
  - 重绘思维导图时，生成完成后显示预览Modal，不立即替换原内容
  - 提供"替换"和"放弃"两个选项，用户确认后才更新内容
  - 保护原有内容，避免误操作导致数据丢失
